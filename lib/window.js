...
class window extends EventEmitter {
	constructor(options = {}) {
		...
		options.w = options.w >= 0 ? options.w : 640
		options.h = options.h >= 0 ? options.h : 480
		...
		let flags = 0
		flags |= options.kiosk ? SDL_video.SDL_WindowFlags.SDL_WINDOW_FULLSCREEN : 0
		flags |= options.fullscreen ? SDL_video.SDL_WindowFlags.SDL_WINDOW_FULLSCREEN_DESKTOP : 0
		...
		this._window = SDL_video.SDL_CreateWindow(options.title, options.x, options.y, options.w, options.h, flags)
		...
		this.on('_event', (event) => {
			if(event.type == SDL_events.SDL_EventType.SDL_WINDOWEVENT) {
				...
				this.emit('editing', str, event.edit.start, event.edit.length)
			}
	...
	_change(w, h) {
		...
		this.render.color = this._options.background
		this.render.clear()
	...
	get focus() {
		return window.focused && (this.id == window.focused.id)
	}
	set grab(_grab) {
	...
	set fullScreen(full) {
		...
		SDL_video.SDL_SetWindowFullscreen(this._window, full)
	}
	get fullScreen() {
	...
	set border(_border) {
		SDL_video.SDL_SetWindowBordered(this._window, !!_border)
	}
	get bordersSize() {
	...
	static showMessageBox(options = {}, callback = () => {}) {
		...
		else if(options.window._window) {
